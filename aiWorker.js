...
let bestMove=null;
let startTime=0;
// simpler settings for Monte Carlo search
const TIME_LIMIT=1000; // max thinking time in ms
const SIM_DEPTH=40; // playout depth
...
function monteCarlo(state){
  ...
  const stats=legal.map(m=>({move:m,wins:0,plays:0}));
  let i=0,bestScore=-Infinity;
  while(!stop && Date.now()-startTime<TIME_LIMIT){
    const st=stats[i];
    const s=clone(state);
    applyMove(s,st.move);
    const r=playout(s,state.turn);
    st.plays++; if(r>0)st.wins++;
    const score=st.wins/st.plays;
    if(score>bestScore){bestScore=score;bestMove=st.move;}
    i=(i+1)%stats.length;
  }
  return bestMove||stats[0].move;
}

function playout(s,root){
  let depth=0;
  while(depth<SIM_DEPTH){
    const moves=generateLegalMoves(s,s.turn);
    if(moves.length===0) return s.turn===root? -1:1;
    const mv=moves[Math.floor(Math.random()*moves.length)];
    applyMove(s,mv);
    depth++;
  }
  const val=evalState(s)*(root? -1:1);
  return val>0?1:-1;
}
